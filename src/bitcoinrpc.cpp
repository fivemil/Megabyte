...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop DvoraKoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "DvoraKoin server stopping";
}
...
//
// JSON-RPC protocol.  DvoraKoin speaks version 1.0 for maximum compatibility,
...
void StartRPCThreads()
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 62620));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "62620")))
